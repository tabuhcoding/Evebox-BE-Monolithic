...
import { startOfWeek, endOfWeek, addWeeks, differenceInCalendarMonths, differenceInCalendarDays, startOfMonth, subMonths } from 'date-fns';
...
export class GetOrgRevenueChartService {
  ...
  async execute(email: string, fromDate?: string, toDate?: string, filterType: "month" | "year" = "month"): Promise<Result<RevenueSummaryItem[], Error>> {
    try {
      ...
      const orders = await this.getPaidOrdersByShowingIdService.executeAll(from, to);
