...
export class UpdateEventMemberService {
  ...
  async execute(
    ...
    try {
      ...
      const canManage = await this.eventUserRepo.hasPermissionToManageMembers(eventId, user.email.value, currentEmail);
      if (!canManage) return Err(new Error('You do not have permission to manage members.'));

      const updated = await this.eventUserRepo.updateMember(eventId, addedUser.email.value, dto);
